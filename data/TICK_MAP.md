...
* ✅ найден настоящий тик артефакта (второй билд, машина 9400..9387): тело под
  `pcall` в цикле с `task.wait(0.15)`: `cKb[66]()` → `cKb[99]["resourceTick"]()`
  → `cKb[99]["parry"]["step"]()` (F288) → `(noStun or noRagdoll)` → `cKb[88]()`
  → `noRagdoll` → `cKb[140]()` → «медленная ходьба» (`CombatPresets.slow_walk_speed`,
  вернуть WalkSpeed 16) → `bon(); bpX(); boZ()` (AntiAfk + порог убийства +
  сундуки). В сборке это `M.Tick()`/`M.StartTick()`; период 0.15 (пул 4706),
  нормальная скорость 16 (пул 4396).
