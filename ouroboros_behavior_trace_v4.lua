--[[ ============================================================================
  NZL Studio — Ouroboros behavior recorder v4  (targeted + prompts)
  ----------------------------------------------------------------------------
  ...
  Что нового против v3:
    * хуки ProximityPrompt — именно через них Ouroboros подбирает лут и открывает
      сундуки (в артефакте loot-шаг делает obj:FindFirstChildWhichIsA("ProximityPrompt")),
      поэтому сигналов в v2/v3 и не было:
          - события ProximityPromptService (PromptShown/HoldBegan/HoldEnded/Triggered)
          - fireproximityprompt (если исполнитель его даёт)
          - hookfunction на InputHoldBegin/InputHoldEnd каждого промпта (если можно)
          - подписка на новые промпты в workspace (дропы появляются после убийства)
    * RemoteFunction.InvokeServer тоже логируется
...
local function scanRemotes(root, depth)
    ...
    for _, obj in ipairs(root:GetChildren()) do
        local ok = pcall(function()
            ...
            if (isRemote or isFunc) and type(hookfunction) == "function" and not TAGS[obj] then
                TAGS[obj] = true
                local method = isFunc and "InvokeServer" or "FireServer"
                local old = obj[method]
                local new = function(self, ...)
                    log("REMOTE" .. (isFunc and "_INVOKE " or " ") .. obj:GetFullName(), ...)
                    return old(self, ...)
                end
...
-- ---------------------------------------------------------------------------
-- 3.5 ProximityPrompt: лут и сундуки Ouroboros берёт именно ими
--     (в артефакте: aBP:FindFirstChildWhichIsA("ProximityPrompt"), статус
--      bpz["LootStatus"] = "Collecting loot")
local promptTag = {}

local function describePrompt(p)
    local ok, txt = pcall(function()
        return string.format("%s | %s | hold=%.2f | enabled=%s | claimed=%s | parent=%s",
            tostring(p.ActionText), tostring(p.ObjectText), p.HoldDuration or -1,
            tostring(p.Enabled), tostring(p:GetAttribute("DropClaimedBy")),
...
local function hookPrompt(p)
    if promptTag[p] then return end
    promptTag[p] = true
    log("PROMPT_SEEN " .. describePrompt(p))
    if type(hookfunction) == "function" then
        for _, method in ipairs({ "InputHoldBegin", "InputHoldEnd" }) do
            pcall(function()
                local old = p[method]
                if type(old) ~= "function" then return end
                local new = function(self, ...)
                    log("PROMPT " .. method .. " " .. describePrompt(self))
                    return old(self, ...)
                end
                hookfunction(old, new)
                HOOKED += 1
                log("HOOKED PROMPT " .. method)
            end)
        end
    end
    pcall(function()
        p.Triggered:Connect(function()
            log("PROMPT Triggered " .. describePrompt(p))
        end)
    end)
end
...
do
    ...
    for _, ev in ipairs({ "PromptShown", "PromptHidden", "PromptButtonHoldBegan",
                          "PromptButtonHoldEnded", "PromptTriggered", "PromptTriggerEnded" }) do
        pcall(function()
            PPS[ev]:Connect(function(prompt, ...)
                log("PROMPTSVC " .. ev .. " " .. describePrompt(prompt), ...)
            end)
        end)
    end
    local fired = false
    pcall(function()
        ...
        fireproximityprompt = function(p, ...)
            log("FIREPROMPT " .. describePrompt(p), ...)
            return old(p, ...)
        end
        fired = true
        HOOKED += 1
        log("HOOKED fireproximityprompt")
    end)
    if not fired then log("fireproximityprompt: нет в этом исполнителе") end

    local function scanPrompts(root, depth)
        if depth > 5 or not root then return end
        for _, obj in ipairs(root:GetChildren()) do
            pcall(function()
                if obj:IsA("ProximityPrompt") then hookPrompt(obj) end
            end)
            scanPrompts(obj, depth + 1)
        end
    end
    local okP = pcall(scanPrompts, workspace, 0)
    log("PROMPT_SCAN " .. tostring(okP))
    pcall(function()
        workspace.DescendantAdded:Connect(function(obj)
            if obj:IsA("ProximityPrompt") then
                hookPrompt(obj)
                log("PROMPT_ADDED " .. describePrompt(obj))
            end
        end)
    end)
end
